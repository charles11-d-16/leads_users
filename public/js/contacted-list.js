console.log("contacted-list.js loaded");
 ...
 let sortNameAsc = true;
    ...
    document.getElementById('statusForm').onsubmit = async function(e) {
        ...
        if (!inquiryId) {
            showModalMessage('No lead selected');
            return;
        }
...
function renderContactedInquiries(inquiries) {
  const tbody = document.getElementById('leadsBody');
  tbody.innerHTML = inquiries.map(inq => {
    ...
    return `
      <tr onclick="showDetails(
        ...
        <td><span class="status-badge bg-contacted">${inq.status}</span></td>
...
async function loadContactedInquiries() {
  try {
    const res = await fetch('/api/inquiries?status=Contacted');
    const inquiries = await res.json();
    renderContactedInquiries(inquiries);
  } catch (err) {
    console.error('Error loading contacted inquiries:', err);
  }
}
