console.log("progress-list.js loaded");
...
function renderProgressInquiries(inquiries) {
  const tbody = document.getElementById('leadsBody');
  tbody.innerHTML = inquiries.map(inq => {
    ...
    return `
      <tr onclick="showDetails(
        ...
        <td><span class="status-badge bg-progress">${inq.status}</span></td>
...
async function loadProgressInquiries() {
  try {
    const res = await fetch('/api/inquiries?status=In Progress');
    const inquiries = await res.json();
    renderProgressInquiries(inquiries);
  } catch (err) {
    console.error('Error loading progress inquiries:', err);
  }
}

function showDetails(ref, name, email, type, notes, staff, date) {
